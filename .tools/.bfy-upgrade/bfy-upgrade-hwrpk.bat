...
title BFY Upgrade Tool v2.0 - hwRpk
...
set "CONFIG=%~dp0config-hwrpk.json"
...
if not exist "%CONFIG%" (
    echo [ERROR] config-hwrpk.json not found
    pause
    exit /b 1
)

:: display config info
for /f "delims=" %%a in ('node -e "try{var c=require('%~dp0config-hwrpk.json');console.log(c.source||'');console.log(c.target||'');console.log(c.name||'');}catch(e){}"') do (
    if "!src!"=="" (set "src=%%a") else if "!tgt!"=="" (set "tgt=%%a") else if "!name!"=="" (set "name=%%a")
)

echo.
echo ================================================================
echo   BFY Upgrade Tool v2.0  (Huawei Quick Game / RPK)
echo ================================================================
echo.
echo   Config : %CONFIG%
echo   Source : !src!
...
if errorlevel 1 (
    echo.
    echo ================================================================
    echo   [ERROR] Dry-run failed. Check paths in config-hwrpk.json.
    echo ================================================================
    pause
    exit /b 1
)
